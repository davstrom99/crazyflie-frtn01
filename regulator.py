...
class Regulator(threading.Thread):
    ...
    def __init__(self, link_uri):
        ...
        self.thrust_ctrl = PDController(self.period_in_ms, 37000)
        self.thrust_ctrl.set_params(K=25000, h = float(self.period_in_ms)/1000, Td = 0.4, N=10)
        roll_pitch_K = 15
        roll_pitch_Td = 0.5
        roll_pitch_N = 10
        self.roll_ctrl = PDController(self.period_in_ms)
        self.roll_ctrl.set_params(K=roll_pitch_K, h= float(self.period_in_ms)/1000, Td=roll_pitch_Td,N=roll_pitch_N)
        self.pitch_ctrl = PDController(self.period_in_ms)
        self.pitch_ctrl.set_params(K=roll_pitch_K,h=float(self.period_in_ms)/1000, Td = roll_pitch_Td, N= roll_pitch_N)
    ...
    def thrust_limit(self, sgnl):
        """Limits the thrust control signal and casts it to int"""
        if sgnl>65535:
            out = 65535
        elif sgnl<20000:
            out = 0
    ...
    def pitch_roll_limit(self,sgnl):
        """Limits the roll/pitch control signal"""
        if sgnl>30:
            out = 30
        elif sgnl<-30:
            out = -30
    ...
    def lift_off(self):
        """Makes sure that the crazyflie lifts off without disturbances of the
        horizontal controllers."""
        start_lift_time = time.time()
        while (time.time()-start_lift_time)<1:
            time_start = time.time()
            # Thrust controller
            self.cf.commander.send_setpoint(0, 0, 0, 38000)
            self.loop_sleep(time_start)

    def get_thrust_plot_data(self):
        '''Returns the regulator data in format
        [Reference z-position, Current z-position, Thrust control signal]'''
        return [self.ref[2], self.pos[2], self.thrust_ctrl_signal]

    def get_pitch_plot_data(self):
        '''Returns the regulator data in format
        [Reference x-position, Current x-position, Pitch control signal]'''
        return [self.ref[0], self.pos[0]-self.start_pos[0], self.pitch_ctrl_signal]

    def get_roll_plot_data(self):
        '''Returns the regulator data in format
        [Reference y-position, Current y-position, Roll control signal]'''
        return [self.ref[1], self.start_pos[1]-self.pos[1], self.roll_ctrl_signal]
    ...
    def _run(self):
        ...
        while True:
            ...
            # Thrust controller
            with self._lock:
                self.thrust_ctrl_signal = self.thrust_limit(self.thrust_ctrl.calc_out(self.pos[2],self.ref[2]))
                # Pitch controller
                self.pitch_ctrl_signal = self.pitch_roll_limit(self.pitch_ctrl.calc_out(self.pos[0]-self.start_pos[0],self.ref[0]))
                # Roll controller
                self.roll_ctrl_signal = self.pitch_roll_limit(self.roll_ctrl.calc_out(self.start_pos[1]-self.pos[1],self.ref[1]))
                self.cf.commander.send_setpoint(self.roll_ctrl_signal, self.pitch_ctrl_signal, 0, self.thrust_ctrl_signal)
                # Updating states
                self.thrust_ctrl.update_state(self.thrust_ctrl_signal)
                self.pitch_ctrl.update_state(self.pitch_ctrl_signal)
                self.roll_ctrl.update_state(self.roll_ctrl_signal)
            self.loop_sleep(tid)
